...
class DQN(nn.Module):
    def __init__(self, input_dim, output_dim):
        ...
        self.fc1 = nn.Linear(input_dim, SIZE)  # First hidden layer
        self.fc2 = nn.Linear(SIZE, SIZE)  # Second hidden layer
        self.fc3 = nn.Linear(SIZE, output_dim)  # Output layer (Q-values for each action)
...
class DQNAgent:
    def __init__(self, state_size, action_size, alpha=0.001, gamma=0.99, epsilon=1.0, epsilon_min=0.1, epsilon_decay=0.997):
        ...
        self.memory = deque(maxlen=500)  # Replay memory to store transitions
        self.batch_size = 64
